...
const createBlog = async function (req, res) {
  try {
    ...
    if (!blog.authorId)
      res.status(400).send({ status: false, msg: "Please include an authorId" });
    if (!blog.category)
      res.status(400).send({ status: false, msg: "Please include an category" });
    ...
    res.status(201).send({status: true,msg: "data created successfully",data: blogCreated,});
  } 
  catch (err) {
    ...
    if(data.authorId || data.subcategory ||  data.category ||  data.tags || data.isPublished ) {
       ...
       if (newData.length == 0) return res.status(404).send({status: false, msg: 'BLOG NOT FOUND' });
        for (let i=0; i<newData.length; i++){
          if (newData[i].isDeleted == true) {res.status(400).send({ status: false, msg: "This data is already deleted" }); }
        }
